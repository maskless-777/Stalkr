...
namespace Stalkr.Controllers
{
    ...
    {
        private readonly AttendsRepository _repo;

        public AttendsController(AttendsRepository repo)
        {
            _repo = repo;
        }
        ...
        public async Task<IActionResult> GetByFromPersonId(int fromPersonId)
        {
            ...
            if (!list.Any())
                return NotFound($"No ATTENDS relationships found for person {fromPersonId}");
            return Ok(list);
        }

        [HttpPost]
        public async Task<IActionResult> Create([FromBody] AttendsRelationshipModel rel)
        {
